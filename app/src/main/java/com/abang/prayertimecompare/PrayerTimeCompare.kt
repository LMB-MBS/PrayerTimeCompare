...
class PlaybackService : Service() {
    ...
    override fun onCreate() {
        ...
        // ✅ Wrap NotificationChannel creation (API 26+ only)
        ...
        // ✅ Build notification (use channel only if API 26+)
        ...
        val notification = notificationBuilder
            ...
            .addAction(R.drawable.ic_pause, "Pause", getActionIntent("ACTION_PAUSE"))
            .addAction(R.drawable.ic_stop, "Stop", getActionIntent("ACTION_STOP"))
            .build()

        // ✅ Wrap startForeground for Android 14+ (API 34)
...
private val prayerTabs = mutableMapOf<String, Pair<TextView, ImageView>>()
...
class MainActivity : Activity() {
    ...
    // These are declared at the top of the Activity class using `lateinit`
    // because they will be initialized later inside `onCreate()`.
    // lateinit` allows us to keep them non-null without assigning a value immediately.
    private lateinit var rootLayout: LinearLayout      // Root container for the entire screen
    private lateinit var topTabs: LinearLayout         // Horizontal bar for navigation tabs/buttons
    private lateinit var contentTable: TableLayout
    private lateinit var tableContainer: ScrollView
    private lateinit var debugTextView: TextView  // For debug mode logs only    private lateinit var statusBar: TextView
    private lateinit var statusBar: TextView  // ← Make sure this is here!
    private lateinit var bigCountdownView: TextView
    ...
    private val allLogs = LinkedHashMap<String, String>() // timestamp -> logEntry  Store ALL logs here
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Get the current year (used later for data directories and computations)
        year = Calendar.getInstance().get(Calendar.YEAR)

        // --- Get app version name safely ---
        val appVersion: String = try {
            val packageInfo = packageManager.getPackageInfo(packageName, 0)
            "v${packageInfo.versionName}" // e.g. "v1.0"
        } catch (e: Exception) {
            "v?" // fallback if version not found
        }

        // -------------------------------------------------------------------------
        // 1. Build the root layout (vertical orientation, dark background)
        // -------------------------------------------------------------------------
        rootLayout = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            setBackgroundColor(Colors.BG) // Custom color constant for background
        }

        // -------------------------------------------------------------------------
        // 2. Create and add the title bar at the top
        // -------------------------------------------------------------------------
        val title = TextView(this).apply {
            text = "PrayerTimeCompare $appVersion"           // App title text
            textSize = 20f                        // Larger font size for visibility
            setTextColor(Colors.TITLE_TEXT)       // Custom color constant for title text
            setPadding(16.dpToPx(), 16.dpToPx(), 16.dpToPx(), 8.dpToPx()) // Padding around text
            setBackgroundColor(Colors.TITLE_BG)   // Custom background color for title bar
            typeface = android.graphics.Typeface.DEFAULT_BOLD // Bold font style
        }
        rootLayout.addView(title) // Add title bar to the root layout

        // -------------------------------------------------------------------------
        // 3. Create and add the top tab bar (horizontal layout for navigation)
        // -------------------------------------------------------------------------
        topTabs = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            setPadding(8.dpToPx(), 8.dpToPx(), 8.dpToPx(), 8.dpToPx()) // Padding inside tab bar
            setBackgroundColor(Color.DKGRAY) // Dark gray background for contrast
        }
        rootLayout.addView(topTabs) // Add tab bar to the root layout

        // -------------------------------------------------------------------------
        // 4. Create the scrollable content area WITH TABLE LAYOUT
        // -------------------------------------------------------------------------
        // ScrollView that fills remaining vertical space
        tableContainer = ScrollView(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                0,   // height = 0dp
                1.0f // weight = 1 → fills remaining space
            )
            setBackgroundColor(Colors.BG)
        }


        contentTable = TableLayout(this).apply {
            layoutParams = TableLayout.LayoutParams(
                TableLayout.LayoutParams.MATCH_PARENT,
                TableLayout.LayoutParams.WRAP_CONTENT
            )
            isStretchAllColumns = true  // CRITICAL: Makes columns fill width
            setBackgroundColor(Colors.BG)
        }

        // Add debug TextView (hidden by default, shown only in debug mode)
        ...
        // Create a container for debug text + table
        ...
        contentContainer.addView(debugTextView)
        contentContainer.addView(contentTable)
        tableContainer.addView(contentContainer)
        rootLayout.addView(tableContainer)

        // -------------------------------------------------------------------------
        // 5. Create and add the NEW status bar at the bottom
        // -------------------------------------------------------------------------
        // Create a container for status bar + settings icon
        ...
        // Status text (left side - takes most space)
        statusBar = TextView(this).apply {
            ...
            layoutParams = LinearLayout.LayoutParams(
                0,
                LinearLayout.LayoutParams.WRAP_CONTENT,
                1.0f  // Takes all available space
            )
            gravity = Gravity.CENTER_VERTICAL
        }
        ...
        statusContainer.addView(settingsIcon)

        rootLayout.addView(statusContainer)

        // -------------------------------------------------------------------------
        // 6. Set the root layout as the content view for the Activity
        // -------------------------------------------------------------------------
        ...
        // -------------------------------------------------------------------------
        // 7. Initialize directories and build tab UI
        // -------------------------------------------------------------------------
        setupDirectories(year) // Ensure working/backup directories exist for this year
        renderInitialTable()
        ensureDefaultGraceSettings()
        //renderTableFromBuffer()
        buildTabs()            // Populate the topTabs layout with tab buttons
        //showPrayer(activePrayer)
        setupCountdownArea()

        // FINALLY setup debug toggle after UI is fully initialized
        setupDebugToggle()
        setupTitleLongPress()

        // Just log the initial config (optional)
        ...
        // -------------------------------------------------------------------------
        // 8. Start background thread to load shared data
        // -------------------------------------------------------------------------
        // Show placeholder immediately before starting heavy work
        showLoadingPlaceholder()

        Thread {
            try {
                appendLine("START: loading shared data for $year")
                loadSharedDataAndComputeIfNeeded() // Perform heavy data loading/computation
                ...
                // Start auto-mode checker
                startSynchronizedAutoChecker()

                // AUTO-DETECT current prayer before showing UI
                val (detectedPrayer, _) = getNextPrayerAndTime()

                // Once data is ready, update UI on the main thread
                runOnUiThread {
                    activePrayer = detectedPrayer  // ← Set to actual current prayer
                    updateTabStyles()
                    renderTableFromBuffer()        // 👈 replaces placeholder with actual table
                    dataReady = true   // 👈 mark data as ready

                    appendLine("Auto-detected prayer: $activePrayer")

                    // CLEAR "Initializing..." or "Ready" message
                    statusBar.text = ""
                    currentStatusBarPriority = STATUS_BAR_PRIORITIES.NORMAL

                    // Start countdown AFTER UI is fully updated
                    startAdaptiveSynchronizedCountdown()
                }
            } catch (e: Exception) {
                // Log crash details if something goes wrong
                appendLine("CRASH: ${e.message}")
                e.printStackTrace()

                // Show placeholder again if computation fails
                runOnUiThread {
                    showLoadingPlaceholder()
                }
            }
    ...
    override fun onDestroy() {
        ...
        // Stop auto-mode checker
        handler.removeCallbacks(autoModeChecker)

        // Stop blinking handler
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            ).apply { description = "Channel for prayer time azan playback" }
    ...
    private fun startGracePeriod(prayer: String) {
        ...
        runOnUiThread {
            ...
            bigCountdownView.text = "🕌\n$prayer TIME"
            bigCountdownView.setTextColor(Color.GREEN)
            bigCountdownView.visibility = View.VISIBLE  // IMPORTANT: Keep it visible!
            ...
            // 4. Update status bar
            statusBar.text = "Switching to next prayer in ${graceSeconds}s"
        }
    ...
    private fun endGracePeriod() {
        ...
        runOnUiThread {
            ...
            bigCountdownView.text = ""
            bigCountdownView.visibility = View.GONE
    ...
    private fun debugLayoutStructure() {
        runOnUiThread {
            ...
            appendLine("Root layout has ${rootLayout.childCount} children:")
    ...
    private fun buildTabs() {
        ...
        val tabWeights = listOf(0.18f, 0.22f, 0.15f, 0.28f, 0.17f)

        // Define background colors for each prayer button
        val tabColors = listOf(
            Color.parseColor("#FFCDD2"), // Fajr (soft red/pink)
            Color.parseColor("#C8E6C9"), // Dhuhr (light green)
            Color.parseColor("#BBDEFB"), // Asr (light blue)
            Color.parseColor("#FFF9C4"), // Maghrib (light yellow)
            Color.parseColor("#D1C4E9")  // Isha (lavender/purple)
        )

        for ((index, p) in Prayers.ORDER.withIndex()) {
            ...
            val label = TextView(this).apply {
                text = p
                gravity = Gravity.CENTER
                setTextColor(Colors.TAB_INACTIVE)
                textSize = 16f
            ...
            tabLayout.addView(speakerIcon)

            val lp = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, tabWeights[index])
            topTabs.addView(tabLayout, lp)

            // Save references for later updates
            prayerTabs[p] = label to speakerIcon
        }
        updateTabStyles()
    }
    ...
    private fun startReturnCountdown() {
        ...
        returnCountdownTask = object : Runnable {
            override fun run() {
                ...
                val timeLeft = (userOverrideExpiry - System.currentTimeMillis()) / 1000

                if (timeLeft > 0) {
                    updateStatusBarWithPriority(
                        "Returning to auto-mode in ${timeLeft}s",
                        STATUS_BAR_PRIORITIES.MANUAL_OVERRIDE
                    )
                    returnCountdownHandler?.postDelayed(this, 1000)
                } else {
                    // TIMER EXPIRED - return to auto-mode
                    appendLine("20s timer expired - returning to auto-mode")
                    returnToCurrentPrayer()  // ← This will restart prayer countdown
                }
            }
        }
    ...
    private fun updateTabStyles() {
        for (i in 0 until topTabs.childCount) {
            val tabLayout = topTabs.getChildAt(i) as LinearLayout
            val label = tabLayout.getChildAt(0) as TextView
            val speakerIcon = tabLayout.getChildAt(1) as ImageView
            val prayerName = Prayers.ORDER[i]

            if (prayerName.equals(activePrayer, ignoreCase = true)) {
                // ACTIVE TAB styling
                label.text = prayerName.uppercase(Locale.getDefault())
                label.setTextColor(Colors.TAB_ACTIVE)
                label.setPadding(8.dpToPx(), 12.dpToPx(), 8.dpToPx(), 6.dpToPx())
                label.setTypeface(null, Typeface.BOLD)
                label.textSize = 15f

                // Speaker icon only visible if playback is active
                if (isPlaying) {
                    speakerIcon.visibility = View.VISIBLE
                } else {
                    speakerIcon.visibility = View.GONE
                }
            } else {
                // INACTIVE TAB styling
                label.text = prayerName
                label.setTextColor(Colors.TAB_INACTIVE)
                label.setPadding(8.dpToPx(), 8.dpToPx(), 8.dpToPx(), 8.dpToPx())
                label.setTypeface(null, Typeface.BOLD)
                label.textSize = 14f

                // Always hide speaker icon for inactive prayers
                speakerIcon.visibility = View.GONE
            }
        }
    }
    ...
    private fun showPrayer(prayer: String) {
        ...
        runOnUiThread {
            ...
            appendLine("DEBUG contentTable visibility: ${contentTable.visibility}")
            appendLine("DEBUG contentTable width: ${contentTable.width}, height: ${contentTable.height}")

            if (DEBUG_MODE) {
                // Debug mode: show logs in debugTextView, hide table
                debugTextView.visibility = View.VISIBLE
                contentTable.visibility = View.GONE

                val allLogsText = allLogs.values.joinToString("\n")
                debugTextView.text = allLogsText + "\n=== Displaying prayer: $prayer ==="
            } else {
                // Normal mode: show table, hide debug text
                debugTextView.visibility = View.GONE
                contentTable.visibility = View.VISIBLE
                ...
                contentTable.post {
                    ...
                    tableContainer.fullScroll(View.FOCUS_UP)
                }
            }
        }
    }
    ...
    private fun addSeparatorRow(numColumns: Int) {
        ...
        contentTable.addView(separatorRow)
    }
    ...
    private fun buildPrayerTable(prayer: String, dates: List<Date>) {
        ...
        runOnUiThread {
            appendLine("DEBUG In runOnUiThread, contentTable child count: ${contentTable.childCount}")
            // Clear existing table
            contentTable.removeAllViews()
            appendLine("DEBUG Table cleared, new child count: ${contentTable.childCount}")
            ...
            appendLine("DEBUG Table built, final child count: ${contentTable.childCount}")
    ...
    private fun appendLine(s: String) {
        runOnUiThread {
            ...
            if (DEBUG_MODE) {
                val allLogsText = allLogs.values.joinToString("\n")
                debugTextView.text = allLogsText
                tableContainer.post { tableContainer.fullScroll(View.FOCUS_DOWN) }
            }
        }
    }
    ...
    private fun setupDebugToggle() {
        val title = rootLayout.getChildAt(0) as? TextView
        title?.setOnClickListener {
            ...
            if (debugTapCount in 3..4) {
                statusBar.text = "Debug: ${debugTapCount}/5"
                handler.removeCallbacks(resetDebugCounterTask)
                handler.postDelayed({
    ...
    private fun enterDebugMode() {
        ...
        // Set up debug display first
        showPrayer(activePrayer) // displays append logs before Table using appendLine()

        // Then set persistent status (won't be overridden)
        statusBar.setBackgroundColor(Color.RED)
        val rmse = annualRMSE[activePrayer] ?: 0.0
        statusBar.text = "M85 RMSE: ${"%.2f".format(rmse)}min - Click any tab to exit"
                    ...
                    if (todayStr != lastDateStr) {
                        ...
                        if (method99Map.isNotEmpty() && method12Map.isNotEmpty()) {
                            runOnUiThread {
                                ...
                                appendLine("Midnight refresh: Fajr table rebuilt, rows=${contentTable.childCount}")
                            }
                        } else {
...
// ==================== BASIC COUNTDOWN VIEW ====================
    ...
    private fun setupCountdownArea() {
        // Create container for prayer countdown
        ...
        // Prayer countdown (always visible except during grace)
        bigCountdownView = TextView(this).apply {
            textSize = 50f
            gravity = Gravity.CENTER
            typeface = Typeface.MONOSPACE
            ...
            setBackgroundColor(Color.TRANSPARENT)
            visibility = View.VISIBLE  // Always visible by default
        ...
        countdownContainer.addView(bigCountdownView)
        rootLayout.addView(countdownContainer, 2)  // After tabs, before scroll

        // Create grace timer view (initially hidden, positioned after table)
        graceTimerView = TextView(this).apply {
            textSize = 48f
            gravity = Gravity.CENTER
            typeface = Typeface.MONOSPACE
            setTypeface(typeface, Typeface.BOLD)
            setTextColor(Color.RED)
            setBackgroundColor(Color.parseColor("#30000000"))  // Semi-transparent black
        ...
        // Add grace timer at position 4 (after table container)
        rootLayout.addView(graceTimerView, 4)
    }

    private fun showBigCountdown(seconds: Int, prayer: String) {
        runOnUiThread {
            if (!::bigCountdownView.isInitialized) return@runOnUiThread
            ...
            bigCountdownView.text = displayText
            bigCountdownView.setTextColor(getGracePeriodColor(seconds, prayer))
            bigCountdownView.visibility = View.VISIBLE
        }
    }
    ...
    private fun updateCountdownWithSeconds(prayer: String, totalSecondsRemaining: Int) {
        runOnUiThread {
            ...
            if (orientation == Configuration.ORIENTATION_PORTRAIT) {
                bigCountdownView.textSize = 50f
                bigCountdownView.text = "$timeLine\n$prayerLine"   // two lines
                (bigCountdownView.layoutParams as LinearLayout.LayoutParams).weight = 0.2f
            } else {
                bigCountdownView.textSize = 25f
                bigCountdownView.text = "$timeLine $prayerLine"    // one line
                (bigCountdownView.layoutParams as LinearLayout.LayoutParams).weight = 0.1f
            }
            ...
            bigCountdownView.setTextColor(color)
            bigCountdownView.visibility = View.VISIBLE

            rootLayout.requestLayout() // force layout refresh
        }
    }
...
// ============================================
    ...
    class PrayerAlarmReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            MainActivity.appendLogEntry("Alarm received for $prayer at $prayerTime")

            val serviceIntent = Intent(context, MainActivity.PrayerNotificationService::class.java).apply {
    ...
    class PrayerNotificationService : Service() {
        ...
        override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            if (intent?.action == "STOP_AZAN") {
                MainActivity.appendLogEntry("Azan stopped by user")
                mediaPlayer?.release()
                mediaPlayer = null
                stopForeground(true)
                stopSelf()
            ...
            val prayer = intent?.getStringExtra("prayer") ?: "Fajr"
            MainActivity.appendLogEntry("Playing azan for $prayer")
        ...
        private fun playAzan(prayer: String) {
            try {
                ...
                mediaPlayer = MediaPlayer.create(this, audioRes).apply {
    ...
    private fun showAlarmPermissionWarning() {
        runOnUiThread {
            // Show in status bar
            statusBar.text = "⚠️ Enable exact alarm permission for precise notifications"
            ...
            handler.postDelayed({
                statusBar.text = ""
                statusBar.setTextColor(Colors.TITLE_TEXT)
    ...
    private fun updateCountdownFormatting(timeText: String, prayerName: String) {
        ...
        if (orientation == Configuration.ORIENTATION_PORTRAIT) {
            bigCountdownView.textSize = 50f
            bigCountdownView.text = "$timeText\nto $prayerName"
            (bigCountdownView.layoutParams as LinearLayout.LayoutParams).weight = 0.2f
        } else {
            bigCountdownView.textSize = 25f
            bigCountdownView.text = "$timeText to $prayerName"
            (bigCountdownView.layoutParams as LinearLayout.LayoutParams).weight = 0.1f
        }
